...
> [!TIP]
> **Mục tiêu:** dùng Claude Code như một workflow kỹ thuật có kiểm soát: hiểu context/token/cache, dùng slash command đúng lúc, biết khi nào clear/compact/plan, và tự động hoá bằng memory, hooks, agents, MCP.

| Chủ đề | Bạn sẽ nắm được |
|---|---|
| 🧠 Mental model | Claude Code đọc context như thế nào, vì sao token tăng |
| 💸 Token/cost | Cách giảm context, tận dụng cache, chọn model phù hợp |
| 🧰 Commands/CLI | Slash commands, terminal commands, custom commands |
| 🪝 Automation | Hooks, permissions, background tasks, headless mode |
| 🤖 Scale workflow | Subagents, MCP, skills, worktree, CI |

---

## Mục lục nhanh

- [0. Hiểu khái niệm cơ bản trước](#0-hiểu-khái-niệm-cơ-bản-trước-đọc-trước-khi-đi-sâu)
- [1. Tất cả Slash Commands](#1-tất-cả-slash-commands-lệnh-trong-phiên-chat)
- [2. CLI commands](#2-cli-commands-chạy-ngoài-terminal)
- [3. Cách dùng tiết kiệm token](#3-cách-dùng-tiết-kiệm-token-quan-trọng-nhất)
- [4. Tính năng quan trọng khác cần nắm](#4-tính-năng-quan-trọng-khác-cần-nắm)
- [5. Lifecycle của Claude Code](#5-lifecycle-của-claude-code--chuyện-gì-xảy-ra-khi-bạn-gõ-claude)
- [6. Cấu trúc thư mục `.claude/`](#6-cấu-trúc-thư-mục-claude-của-một-dự-án)
- [7. Lộ trình đề xuất để thành thạo](#7-lộ-trình-đề-xuất-để-thành-thạo)
- [8. Quick reference](#8-quick-reference--best-practices-tóm-tắt)
- [10. TL;DR](#10-tldr--nếu-bạn-chỉ-nhớ-3-thứ)
...
>
> Skill như "đọc cuốn sách hướng dẫn" — Claude vẫn là Claude, học thêm cách làm.
>
> Agent như "nhờ đồng nghiệp khác làm hộ" — người khác làm, trả về kết quả tóm tắt.

### 0.6 Chọn đúng công cụ theo tình huống

| Tình huống | Nên dùng | Lý do |
|---|---|---|
| Chuyển sang task mới | `/clear` | Giảm context nhiễu và token |
| Session dài nhưng vẫn cần giữ ý chính | `/compact` | Giữ summary, bỏ chi tiết thừa |
| Task lớn/chưa chắc hướng làm | Plan Mode | Review kế hoạch trước khi sửa |
| Cần convention lặp lại | `CLAUDE.md` gọn | Claude luôn đọc khi làm trong repo |
| Cần validate tự động | Hooks | Rule không phụ thuộc Claude nhớ hay quên |
| Cần tìm kiếm rộng | Subagent Explore | Trả về summary thay vì nhồi context chính |
| Cần tool ngoài | MCP | Kết nối Playwright/Figma/API/tooling |
...
> [!IMPORTANT]
> Nếu chỉ nhớ một nguyên tắc token: **đổi task thì `/clear`, task lớn thì plan trước, convention dài thì đừng nhét hết vào `CLAUDE.md`.**
...
> [!WARNING]
> Những pattern dưới đây làm Claude đọc nhiều context hơn cần thiết, khiến session chậm, đắt và dễ nhiễu.
...
> [!WARNING]
> Mỗi MCP server thêm vào = thêm tool definitions vào system prompt = tốn token mỗi prompt. Chỉ cài cái bạn dùng.
...
> [!WARNING]
> Tổng cộng các file này load vào **mỗi request**. CLAUDE.md của dự án bạn dài ~300 dòng → khoảng 3-5k token mỗi lượt.
...
| ✅ DO | ❌ DON'T |
|---|---|
| `/clear` giữa các task khác nhau | Để session chạy cả ngày không clear |
| Cung cấp đường dẫn file cụ thể | Hỏi "explore codebase" mỗi lần mở session |
| Dùng plan mode cho task lớn | Sửa CLAUDE.md liên tục giữa các lượt (vỡ cache) |
| Commit CLAUDE.md vào repo | Dùng Opus cho việc rename biến |
| Dùng subagent cho việc tìm kiếm rộng | Bỏ qua plan mode khi không chắc |
| Chọn model phù hợp độ khó | `--dangerously-skip-permissions` trên repo thật |
...
> [!NOTE]
> Tài liệu này dành cho Claude Code version 2026, model mặc định Opus 4.7 / Sonnet 4.6 / Haiku 4.5. Một số tính năng có thể đổi khi update — luôn check `/release-notes`.
